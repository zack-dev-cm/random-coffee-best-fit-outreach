...
This repository ships one small public-safe surface: a deterministic random coffee matcher plus a Codex skill for reviewed LinkedIn or Discord outreach.

## Operating Rules

- Restate the cohort goal, consent boundary, target outreach surface, and verification step before editing.
- Keep diffs narrow. Do not add scraping, selfbot, or auto-send behavior.
...
- `skill/random-coffee-best-fit-outreach/`: Codex/OpenClaw/Computer Use skill wrapper.
...
3. Run `python3.10 -m codex_harness audit . --strict --min-score 90` from a checkout that has `codex_harness` available.
